...
class DialogueManager(threading.Thread):
	...
	def GetUserAction(self, agentAction):
		if not REAL_USER:
			...
			userAction, reward, result = self.user.GetNextAction(math.ceil(len(self.stateTracker.history)/2), agentAction)
		else:
	...
	def FillWithMatch(self, nextAgentAction):
		...
		if possibleEntries:
			# Choose random topic from the possible entries
			chosenEntry = random.choice(possibleEntries)
			# Ensure the chosen entry contains the 'category' key
			if 'category' in chosenEntry:
				# Set match in the filled slots
				self.stateTracker.filledSlots['match'] = chosenEntry['category']
				# Fill agent inform slots with information of the chosen topic
