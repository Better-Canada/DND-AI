	...
	def GetIntent(self, userInput):
		...
		for word in userInput.split():
			if word in self.model.key_to_index:
				...
				confirmProbability += (self.model.similarity(word, 'confirm') \
									+ self.model.similarity(word, 'accept') \
				...
				rejectProbability += (self.model.similarity(word, 'reject') \
									...
									+ self.model.similarity(word, 'not') \
									+ self.model.similarity(word, 'wrong') \
									+ self.model.similarity(word, 'no')) / 5.0
